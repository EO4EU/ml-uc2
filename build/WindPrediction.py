            ...
            try:
                  ...
                  def threadentry(raw_data):
                        ...
                        try:
                              ...
                              logger_workflow.debug('S3Client is ready',extra={'status': 'DEBUG'})
                              if s3_path.endswith('/'):
                                    s3_path=s3_path[:-1]
                              cp = CloudPath("s3://"+s3_bucket_output+'/'+s3_path+'/', client=clientS3)
                              cpOutput = CloudPath("s3://"+s3_bucket_output+'/result-uc2-WindPrediction/')
                              logger_workflow.debug("path is s3://"+s3_bucket_output+'/result-uc2-WindPrediction/',extra={'status': 'DEBUG'})
                              ...
                              with cpOutput.joinpath('log.txt').open('w') as fileOutput:
                                    ...
                                    for file_number, folder in enumerate(to_treat):
                                          ...
                                          asyncio.run(doInference(input,logger_workflow,file_number,total_number,file_timings))
                                          array=[]
                                          for elem in input:
                                    ...
                                    logger_workflow.debug('Output written',extra={'status': 'INFO'})
                                    logger_workflow.debug('Connecting to Kafka',extra={'status': 'INFO'})
